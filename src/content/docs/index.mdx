---
title: The MacroLibX
head:
  - tag: title
    content: Home | MacroLibX
description: The minilibx, but with Vulkan
template: splash
editUrl: false
hero:
  tagline: Rewritten version of the minilibx graphics API used at 42school, using SDL2 & Vulkan.
  image:
    file: ../../assets/logo.png
  actions:
    - text: Getting started
      link: /guides/getting_started/
      icon: right-arrow
      variant: primary
    - text: Sources
      link: https://github.com/seekrs/MacroLibX
      icon: external
      variant: secondary
---

import { Card, CardGrid } from '@astrojs/starlight/components';
import { ScriptComp } from '../../components/ScriptComp';
import { Footer } from '../../components/Footer';
import Seekrz from '../../components/seekrz.astro';

The MacroLibX is a fast, easy to use, cross-platform graphics library to create applications without having to work directly with low level graphics APIs.
It provides a simple window management and primitive tools for drawing onto the window such as displaying images from disk or drawing pixels directly.

<CardGrid>
	<Card title="Lightning fast" icon="rocket">
		Built on top of Vulkan, the MacroLibX takes advantage of its very low-level nature to achieve high performance with great control over available resources.
	</Card>
	<Card title="Strong error management" icon="approve-check-circle">
		Includes a powerful, partially deactivatable, error system informing the user of problems with its code and graphic memory leaks that tools such as Valgrind are unable to detect.
	</Card>
	<Card title="It's all FOSS" icon="open-book">
		Everything in the MacroLibX is entirely free and open source, all available under the MIT license (even the third-party libraries used).
	</Card>
	<Card title="Cross-platform" icon="laptop">
		Designed to be totally cross-platform, it can run on any SDL2-supported platform that supports Vulkan (even the Nintendo Switch ! theoretically... ).
	</Card>
</CardGrid>

<p style="text-align: center;">[Here](https://macrolibx-legacy.kbz8.me) you can access the legacy (v1.3.3) documentation.</p>

<Seekrz title="Brought to you by">
	The best coalition of all the 42 campuses in the world and if you don't agree that's your problem
</Seekrz>

<ScriptComp client:only />

<Footer/>
